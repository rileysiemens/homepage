  ...
  selectTab: function (tabIndex) {
    ...
    if (this.tabs[this.currentTab] === "Home") {
      ...
      history.replaceState(
        "",
        "",
        window.location.pathname + "#" + slugify(this.tabs[this.currentTab])
      );
    }
  },
});
...
const neapolitanData = () => ({
  ...
  getWeight: function () {
    return Math.round(
      (this.getFlour() + this.getWater() + this.getSalt() + this.getYeast()) /
        this.balls
    );
  },
...
const videoGamesData = () => ({
  ...
  shouldPrintYear: function (index) {
    ...
    return (
      getYear(this.games[index - 1].date) != getYear(this.games[index].date)
    );
  },
...
window.addEventListener("load", () => {
  ...
  document
    .querySelector("#gravatar")
    .setAttribute("src", gravatarUrl(first + "@" + last + ".com"));
